...
export const authentication = asyncHandler(async (req, res, next) =>{
    ...
    switch (bearer) {
        case 'admin':
            decode = verfiyToken({token, secretKey:process.env.JWT_ADMIN_TOKEN_KEY })
            break;
        case 'bearer':
            decode = verfiyToken({token, secretKey:process.env.JWT_TOKEN_KEY})
            break;
        default:
            break;
    } 
...
return asyncHandler(
    async (req, res, next) =>{
        if (!(accessRoles.includes(req.user.role))) {
            return next(new Error("you don't have the access(un authorized account)", {cause:403}))
        }
        return next()
    }
)}